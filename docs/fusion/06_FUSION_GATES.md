...
Pour O2, O4 et O7, la preuve mécanique contient `{oracle_id, reviewed_commit, admission_commit, report_path, admitted_sha256, report_verdict, recorded_status}`. Le contrôleur applique [`REVIEW_ADMISSION_REGISTRY.md`](REVIEW_ADMISSION_REGISTRY.md) : le hash est recalculé depuis le blob au commit d'admission, distinct et ancêtre, jamais depuis une valeur auto-déclarée dans la preuve courante. Il vérifie aussi que le rapport cite `reviewed_commit`, que son verdict correspond exactement à `recorded_status`, que `oracle_id` appartient au `Oracle scope` indexé et que le blob contient le marqueur normatif correspondant. Un statut accepté sans admission, avec rapport modifié, mauvais commit, oracle substitué ou verdict divergent bloque P6.
