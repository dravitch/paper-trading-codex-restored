...
Pour O2, O4 et O7, la preuve mécanique contient `{oracle_id, reviewed_commit, admission_commit, report_path, admitted_sha256, report_verdict, registry_commit, registry_blob_sha256, recorded_status}`. Le contrôleur applique [`REVIEW_ADMISSION_REGISTRY.md`](REVIEW_ADMISSION_REGISTRY.md) : hashes du rapport et du registre recalculés depuis leurs commits historiques, distincts et ancêtres, jamais depuis une valeur auto-déclarée courante. Il vérifie la ligne candidate unique, le verdict du rapport et celui de la table au `registry_commit`. Un statut accepté sans admission, avec rapport ou registre modifié, mauvais commit, oracle substitué, marqueur ambigu/dupliqué ou verdict divergent bloque P6.
