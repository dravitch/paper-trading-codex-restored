...
Un commit qui ne modifie pas le fichier n'est pas une nouvelle révision et valide le blob porté via son dernier `C`. Pour chaque merge `M` de la première parenté depuis la genesis normative (exclue) jusqu'à `E` (inclus), les blobs de tous ses parents doivent être identiques au blob du premier parent et le blob de `M` doit lui être identique; un fichier absent est une valeur distincte. Cette vérification porte aussi sur un merge transparent `-s ours` qui n'apparaît pas dans le `rev-list` limité au chemin. Toute divergence produit `ORACLE_ADMISSION_MERGE_CONFLICT`. Une union éventuelle est créée dans un commit linéaire ultérieur et doit satisfaire les règles d'ajout ci-dessus.

Mutants historiques obligatoires : retirer ou modifier un record antérieur, changer son verdict ou son ancre, dupliquer un oracle, traiter un commit non-révision comme `C`, sauter `P`, accepter des parents de merge divergents, modifier le fichier dans un merge ou accepter un merge transparent `-s ours` dont un parent secondaire diverge. Tous bloquent P6. Cette chaîne Git détecte les mutations dans l'histoire observée; elle ne remplace pas la preuve externe d'immuabilité exigée plus bas.
