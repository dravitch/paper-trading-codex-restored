...
Référence: real_data_vs_synth_data_dp.md
"""
...
class DataLoader:
    ...
    def load_csv(filepath: str, reset_index: bool = True) -> pd.DataFrame:
        ...
        # Validate OHLC coherence (CORRIGÉ: logique DeepSeek)
        ...
        logger.info(
            "Loaded %d clean data points from %s. "
            "Price range: $%.2f - $%.2f",
            len(result),
            filepath,
            result["close"].min(),
...
def validate_timeframe_consistency(config: dict, timeframe: str) -> list:
    ...
    ⚠️ IMPORTANT (DeepSeek): Les paramètres optimaux DÉPENDENT du timeframe !

    Grid Ratio (γ) recommandé par timeframe :
    - 1h  : γ = 0.5-3.0%   (mouvement typique horaire)
    - 4h  : γ = 2-4%       (mouvement typique sur 4h)
    - 1d  : γ = 3-15%      (mouvement typique quotidien)
    - 1w  : γ = 10-20%     (mouvement typique hebdo)

    Safety Buffer (β) recommandé :
    - 1h  : β = 1.0-3.0x   (volatilité intraday)
    - 1d  : β = 2.5-8.0x   (gaps overnight, news)
...
def adapt_config_to_timeframe(base_config: dict, source_tf: str, target_tf: str) -> dict:
    ...
    Physique financière (DeepSeek validated):
    - grid_ratio : proportionnel à √(temps) — la volatilité scale en √t
    - safety_buffer : proportionnel au temps (linéaire) avec CAP
    - grid_size : inversement proportionnel (moins de niveaux en daily)

    Exemple 1h → 1d:
        grid_ratio: 1.5% × √24 ≈ 7.3%
        safety_buffer: 1.5 × 24 = 36 → cappé à 8.0 max
        grid_size: 7 × (1/24) → min 2
    ...
    # (sans cap, 1h→1d donne ×24 = absurde)
    ...
    # grid_size : inversement proportionnel (moins de niveaux pour timeframes larges)
